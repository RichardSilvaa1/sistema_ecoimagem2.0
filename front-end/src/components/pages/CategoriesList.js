...
// Tema completo (mantido como está)
...
const CategoriesList = () => {
  ...
  const fetchCategories = async () => {
    setLoading(true);
    try {
      const res = await api.get('/api/financas/categories'); // Corrige a rota
  ...
  const handleDelete = async (id, name) => {
    ...
    setLoading(true);
    try {
      await api.delete(`/api/financas/categories/${id}`); // Corrige a rota
            ...
            {loading ? (
              ...
              <Box sx={{ flex: 1, overflowY: 'auto', maxHeight: 'calc(100vh - 200px)' }}>
                <Table>
                  ...
                  <TableBody>
                    {categories.map((cat) => (
                      <TableRow key={cat.id} sx={{ '&:hover': { background: '#E0F2F1' } }}>
                        <TableCell>{cat.name}</TableCell>
                        <TableCell>{cat.description || 'Sem descrição'}</TableCell>
                        <TableCell>
                          <Button
                            variant="outlined"
