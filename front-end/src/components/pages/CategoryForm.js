...
import { Save, ArrowBack, Category as CategoryIcon } from '@mui/icons-material';
...
const CategoryForm = () => {
  ...
  const fetchCategory = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/api/financas/categories/${id}`); // Corrige a rota
      setValue('name', res.data.name);
      setValue('description', res.data.description);
    } catch (error) {
      toast.error('Erro ao carregar categoria: ' + (error.response?.data?.error || error.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (id) {
        await api.put(`/api/financas/categories/${id}`, data); // Corrige a rota
        setSaveMessage('Categoria atualizada com sucesso!');
      } else {
        await api.post('/api/financas/categories', data); // Corrige a rota
        setSaveMessage('Categoria criada com sucesso!');
      }
      setTimeout(() => {
        setSaveMessage(null);
        navigate('/categorias');
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 2 }}>
              <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ duration: 0.3 }}>
                <CategoryIcon sx={{ mr: 1, color: '#26A69A', fontSize: '1.8rem', '&:hover': { color: '#4DB6AC' } }} />
              </motion.div>
            ...
            <Typography variant="subtitle1" sx={{ textAlign: 'center', mb: 3, fontSize: '0.9rem' }}>
              {id ? 'Atualize os detalhes da categoria.' : 'Crie uma nova categoria financeira.'}
            </Typography>
